...
export default function ToolsTableRow({ tool }: ToolsTableRowProps) {
  ...
  return (
    ...
    >
      ...
      <td className="p-4">
        <span
          className={`inline-flex items-center rounded-full bg-linear-to-r px-3 py-1 text-xs
            font-semibold text-white ${statusConfig.fromColor} ${statusConfig.viaColor}
            ${statusConfig.toColor}`}
        >
          {statusConfig.label}
        </span>
      </td>
