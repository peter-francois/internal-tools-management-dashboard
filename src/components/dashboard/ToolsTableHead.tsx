...
export default function ToolsTableHead({ sortField, sortDir, onSort }: ToolsTableHeadProps) {
  return (
    <thead>
      <tr className="border-b-2 border-gray-100 dark:border-neutral-800">
        {COLUMNS.map(({ label, field }) => (
          <th
            key={field}
            onClick={() => onSort(field)}
            className="cursor-pointer pb-3 pl-4 text-left text-xs font-medium tracking-wide
              text-gray-400 uppercase transition-colors select-none hover:text-gray-600
